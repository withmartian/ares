...
from ares.code_agents import code_agent_base, llms
from ares.code_agents.terminus_2 import json_parser, xml_parser
...
class Terminus2Agent(code_agent_base.CodeAgent):
    ...
    async def run(self, task: str) -> None:
        ...
        while self._turn_count < self.max_turns:
            ...
            try:
                ...
                try:
                ...
                except llms.OutputLengthExceededError as e:
                    ...
                    if salvaged_response:
                        ...
                        _LOGGER.debug("[%d] Output exceeded length but found valid response. Using truncated version.", id(self))
                ...
                if parsed.commands:
                    ...
                    observation = await self._execute_commands(parsed.commands)
                ...
                _LOGGER.debug("[%d] Observation length: %d chars, first 200: %s", id(self), len(observation), observation[:200])
                self._add_message("user", observation)

            except Exception as e:
                _LOGGER.error("[%d] Error during agent execution: %s", id(self), e, exc_info=True)
    ...
    async def _query_llm(self) -> llms.LLMResponse:
        ...
        if self.enable_summarization and len(self._messages) > 3:
            ...
            if estimated_tokens > 200000:
                _LOGGER.warning("[%d] Proactively summarizing: ~%d estimated tokens exceeds safe threshold", 
                               id(self), estimated_tokens)
                ...
                _LOGGER.info("[%d] After proactive summarization: ~%d estimated tokens", 
                            id(self), estimated_tokens_after)

        # Convert messages to the format expected by the LLM client
        messages = []
        for msg in self._messages:
            messages.append({"role": msg["role"], "content": msg["content"]})

        try:
            response = await self.llm_client(llms.LLMRequest(messages=messages))
            ...
            is_context_error = (
                ...
                "tokens exceed" in error_str or
                "input tokens exceed" in error_str or
                ("invalid request parameters" in error_str and len(self._messages) > 10)
            )
            
            if is_context_error:
                if self.enable_summarization and len(self._messages) > 3:
                    _LOGGER.warning("[%d] Context length exceeded (%d messages), attempting Harbor-style 3-step summarization", 
                                   id(self), len(self._messages))
                    ...
                    _LOGGER.info("[%d] Retrying query with summarized context (%d messages)", 
                                id(self), len(self._messages))
    ...
    async def _execute_commands(
        self, commands: list[json_parser.Command]
    ) -> str:
    ...
    def _add_message(self, role: str, content: str) -> None:
        ...
        self._messages.append({"role": role, "content": sanitized})
...
Original Task: {self._original_instruction}
...
Be comprehensive and detailed. The next agent needs to understand everything that has happened so far in order to continue."""
        ...
        try:
            ...
            summary_response = await self.llm_client(
                llms.LLMRequest(messages=summary_messages)
            )
...
{self._limit_output_length(current_screen)}
...
Please begin by asking several questions (at least five, more if necessary) about the current state of the solution that are not answered in the summary from the prior agent. After you ask these questions you will be on your own, so ask everything you need to know."""
        ...
        try:
            ...
            answers_response = await self.llm_client(
                llms.LLMRequest(messages=answer_messages)
            )
        ...
        _LOGGER.info("[%d] Summarization complete. Subagent tokens: prompt=%d, completion=%d",
                    id(self), 
...
class Terminus2AgentFactory(code_agent_base.CodeAgentFactory):
    ...
    def __call__(
        self, container: containers.Container, llm_client: llms.LLMClient
    ) -> Terminus2Agent:

...
def create_attention_knockout_hook(
    source_positions: list[int],
    target_positions: list[int],
) -> Callable[[torch.Tensor, HookPoint], torch.Tensor]:
    """Create a hook that prevents attention from source to target positions.

    This is useful for studying information flow by blocking specific attention patterns.

    Args:
        source_positions: Token positions that should not attend to targets.
        target_positions: Token positions that should not be attended to.

    Returns:
        Hook function that modifies attention patterns.

    Example:
        ```python
        # Prevent position 10 from attending to positions 0-5
        ...
        model.run_with_hooks(
            tokens,
            fwd_hooks=[("blocks.0.attn.hook_pattern", hook)]
        )
        ```
    """

    def attention_knockout_hook(activation: torch.Tensor, hook: HookPoint) -> torch.Tensor:  # noqa: ARG001
        # Attention pattern shape: [batch, head, query_pos, key_pos]
        modified = activation.clone()

        # Set attention weights to zero (or very small value)
        for source_pos in source_positions:
            for target_pos in target_positions:
                modified[:, :, source_pos, target_pos] = 0.0

        # Renormalize attention patterns (they should sum to 1 across key dimension)

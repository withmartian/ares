# Modifications Copyright 2026 Martian.
# Modified from dm_env (https://github.com/google-deepmind/dm_env)
# 2026-01-19: Imported + formatted; fixed imports.
#
...
class ArrayTest(parameterized.TestCase):
    ...
    def testValidateDtype(self, value, is_valid):
        ...
        if is_valid:  # Should not raise any exception.
            ...
            with self.assertRaisesWithLiteralMatch(ValueError, specs._INVALID_DTYPE % (spec.dtype, value.dtype)):
                spec.validate(value)
    ...
    def testValidateShape(self, value, is_valid):
        ...
        if is_valid:  # Should not raise any exception.
            ...
            with self.assertRaisesWithLiteralMatch(ValueError, specs._INVALID_SHAPE % (spec.shape, value.shape)):
                spec.validate(value)
    ...
    def testReplace(self, arg_name, new_value):
        ...
        for attr_name in set(["shape", "dtype", "name"]).difference([arg_name]):
            self.assertEqual(getattr(new_spec, attr_name), getattr(old_spec, attr_name))
    ...
    def testReplaceRaisesTypeErrorIfSubclassAcceptsVarArgs(self):
        ...
        with self.assertRaisesWithLiteralMatch(TypeError, specs._VAR_ARGS_NOT_ALLOWED):
    ...
    def testReplaceRaisesTypeErrorIfSubclassAcceptsVarKwargs(self):
        ...
        with self.assertRaisesWithLiteralMatch(TypeError, specs._VAR_KWARGS_NOT_ALLOWED):
            spec.replace(name="something_else")


class BoundedArrayTest(parameterized.TestCase):
    def testInvalidMinimum(self):
        with self.assertRaisesWithLiteralMatch(ValueError, specs._MINIMUM_INCOMPATIBLE_WITH_SHAPE):
            specs.BoundedArray((3, 5), np.uint8, (0, 0, 0), (1, 1))

    def testInvalidMaximum(self):
        with self.assertRaisesWithLiteralMatch(ValueError, specs._MAXIMUM_INCOMPATIBLE_WITH_SHAPE):
    ...
    def testMinMaxCasting(self, spec_dtype, min_dtype, max_dtype):
        minimum = np.array(0.0, dtype=min_dtype)
        maximum = np.array((3.14, 15.9, 265.4), dtype=max_dtype)
        spec = specs.BoundedArray(shape=(1, 2, 3), dtype=spec_dtype, minimum=minimum, maximum=maximum)
    ...
    def testEqualBroadcastingBounds(self):
        spec_1 = specs.BoundedArray((1, 2), np.float32, minimum=0.0, maximum=1.0)
        spec_2 = specs.BoundedArray((1, 2), np.float32, minimum=[0.0, 0.0], maximum=[1.0, 1.0])
        self.assertEqual(spec_1, spec_2)

    def testNotEqualDifferentMinimum(self):
        spec_1 = specs.BoundedArray((1, 2), np.float32, minimum=[0.0, -0.6], maximum=[1.0, 1.0])
        spec_2 = specs.BoundedArray((1, 2), np.float32, minimum=[0.0, 0.0], maximum=[1.0, 1.0])
        self.assertNotEqual(spec_1, spec_2)
    ...
    def testNotEqualDifferentMaximum(self):
        spec_1 = specs.BoundedArray((1, 2), np.int32, minimum=0.0, maximum=2.0)
        spec_2 = specs.BoundedArray((1, 2), np.int32, minimum=[0.0, 0.0], maximum=[1.0, 1.0])
        self.assertNotEqual(spec_1, spec_2)

    def testIsUnhashable(self):
        spec = specs.BoundedArray(shape=(1, 2), dtype=np.int32, minimum=0.0, maximum=2.0)
        with self.assertRaisesRegex(TypeError, "unhashable type"):
            hash(spec)

    def testRepr(self):
        as_string = repr(specs.BoundedArray((1, 2), np.int32, minimum=73.0, maximum=101.0))
    ...
    def testValidateBounds(self, value, is_valid):
        ...
        if is_valid:  # Should not raise any exception.
            ...
            with self.assertRaisesWithLiteralMatch(
                ValueError, specs._OUT_OF_BOUNDS % (spec.minimum, value, spec.maximum)
            ):
                spec.validate(value)

    @parameterized.parameters(
        # Semi-infinite intervals.
        dict(minimum=0.0, maximum=np.inf, value=0.0, is_valid=True),
        dict(minimum=0.0, maximum=np.inf, value=1.0, is_valid=True),
        dict(minimum=0.0, maximum=np.inf, value=np.inf, is_valid=True),
        dict(minimum=0.0, maximum=np.inf, value=-1.0, is_valid=False),
        dict(minimum=0.0, maximum=np.inf, value=-np.inf, is_valid=False),
        dict(minimum=-np.inf, maximum=0.0, value=0.0, is_valid=True),
        dict(minimum=-np.inf, maximum=0.0, value=-1.0, is_valid=True),
        dict(minimum=-np.inf, maximum=0.0, value=-np.inf, is_valid=True),
        dict(minimum=-np.inf, maximum=0.0, value=1.0, is_valid=False),
        # Infinite interval.
        dict(minimum=-np.inf, maximum=np.inf, value=1.0, is_valid=True),
        dict(minimum=-np.inf, maximum=np.inf, value=-1.0, is_valid=True),
        dict(minimum=-np.inf, maximum=np.inf, value=-np.inf, is_valid=True),
        dict(minimum=-np.inf, maximum=np.inf, value=np.inf, is_valid=True),
        # Special case where minimum == maximum.
        dict(minimum=0.0, maximum=0.0, value=0.0, is_valid=True),
        dict(minimum=0.0, maximum=0.0, value=np.finfo(float).eps, is_valid=False),
    )
    ...
    def testReplace(self, arg_name, new_value):
        ...
        self.assertIsNot(old_spec, new_spec)
        self.assertEqual(getattr(new_spec, arg_name), new_value)
        for attr_name in set(["shape", "dtype", "name", "minimum", "maximum"]).difference([arg_name]):
            self.assertEqual(getattr(new_spec, attr_name), getattr(old_spec, attr_name))

    @parameterized.parameters(
        [
            dict(minimum=1.0, maximum=0.0),
            dict(minimum=[0.0, 1.0], maximum=0.0),
            dict(minimum=1.0, maximum=[0.0, 0.0]),
            dict(minimum=[0.0, 1.0], maximum=[0.0, 0.0]),
        ]
    )
    def testErrorIfMinimumGreaterThanMaximum(self, minimum, maximum):
        with self.assertRaisesWithLiteralMatch(
            ValueError, specs._MINIMUM_MUST_BE_LESS_THAN_OR_EQUAL_TO_MAXIMUM.format(minimum=minimum, maximum=maximum)
        ):
...
class DiscreteArrayTest(parameterized.TestCase):
    ...
    def testInvalidNumActions(self, num_values):
        with self.assertRaisesWithLiteralMatch(ValueError, specs._NUM_VALUES_NOT_POSITIVE.format(num_values)):
            specs.DiscreteArray(num_values=num_values)

    @parameterized.parameters(np.float32, object)
    def testDtypeNotIntegral(self, dtype):
        with self.assertRaisesWithLiteralMatch(ValueError, specs._DTYPE_NOT_INTEGRAL.format(dtype)):
            specs.DiscreteArray(num_values=5, dtype=dtype)

    @parameterized.parameters(dict(dtype=np.uint8, num_values=2**8 + 1), dict(dtype=np.uint64, num_values=2**64 + 1))
    def testDtypeOverflow(self, num_values, dtype):
        with self.assertRaisesWithLiteralMatch(ValueError, specs._DTYPE_OVERFLOW.format(np.dtype(dtype), num_values)):
    ...
    def testReplace(self, arg_name, new_value):
        ...
        self.assertIsNot(old_spec, new_spec)
        self.assertEqual(getattr(new_spec, arg_name), new_value)
        for attr_name in set(["num_values", "dtype", "name"]).difference([arg_name]):
            self.assertEqual(getattr(new_spec, attr_name), getattr(old_spec, attr_name))
...
class StringArrayTest(parameterized.TestCase):
    ...
    def testInvalidStringType(self, string_type):
        with self.assertRaisesWithLiteralMatch(ValueError, specs._INVALID_STRING_TYPE.format(string_type)):
            specs.StringArray(shape=(), string_type=string_type)

    @parameterized.parameters(
        dict(value=["foo", "bar"], spec_string_type=str),
        dict(value=("foo", "bar"), spec_string_type=str),
        dict(value=np.array(["foo", "bar"]), spec_string_type=str),
    ...
    @parameterized.parameters(
        dict(value=np.array("foo"), spec_shape=(1,)),
        dict(value=np.array(["foo"]), spec_shape=()),
        dict(value=np.array(["foo", "bar", "baz"]), spec_shape=(2,)),
    )
    def testInvalidShape(self, value, spec_shape):
        spec = specs.StringArray(shape=spec_shape, string_type=str)
        with self.assertRaisesWithLiteralMatch(ValueError, specs._INVALID_SHAPE % (spec_shape, value.shape)):
            spec.validate(value)

    @parameterized.parameters(
        dict(bad_element=42, spec_string_type=str),
        dict(bad_element=False, spec_string_type=str),
        dict(bad_element=["foo"], spec_string_type=str),
        dict(bad_element=b"foo", spec_string_type=str),
        dict(bad_element="foo", spec_string_type=bytes),
    )
    def testInvalidItemType(self, bad_element, spec_string_type):
        ...
        message = specs._INVALID_ELEMENT_TYPE % (spec_string_type, bad_element, type(bad_element))
        with self.assertRaisesWithLiteralMatch(ValueError, message):
            spec.validate(value)

    @parameterized.parameters(
        dict(shape=(), string_type=str, expected=np.array("", dtype=object)),
        dict(shape=(1, 2), string_type=bytes, expected=np.array([[b"", b""]], dtype=object)),
    )

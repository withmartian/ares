...
class Array:
    ...
    def __repr__(self):
        return "Array(shape={}, dtype={}, name={})".format(self.shape, repr(self.dtype), repr(self.name))
...
class BoundedArray(Array):
    ...
    def __init__(self, shape, dtype, minimum, maximum, name=None):
        ...
        if np.any(bcast_minimum > bcast_maximum):
            raise ValueError(_MINIMUM_MUST_BE_LESS_THAN_OR_EQUAL_TO_MAXIMUM.format(minimum=minimum, maximum=maximum))
    ...
    def __repr__(self):
        template = "BoundedArray(shape={}, dtype={}, name={}, minimum={}, maximum={})"
        return template.format(self.shape, repr(self.dtype), repr(self.name), self._minimum, self._maximum)
    ...
    def generate_value(self):
        return np.ones(shape=self.shape, dtype=self.dtype) * self.dtype.type(self.minimum)

    def __reduce__(self):
        return BoundedArray, (self._shape, self._dtype, self._minimum, self._maximum, self._name)


_NUM_VALUES_NOT_POSITIVE = "`num_values` must be a positive integer, got {}."
_DTYPE_NOT_INTEGRAL = "`dtype` must be integral, got {}."
_DTYPE_OVERFLOW = "`dtype` {} is not big enough to hold `num_values` ({}) without overflow."
...
class DiscreteArray(BoundedArray):
    ...
    def __init__(self, num_values, dtype=np.int32, name=None):
        ...
        super(DiscreteArray, self).__init__(shape=(), dtype=dtype, minimum=0, maximum=maximum, name=name)
...
_INVALID_STRING_TYPE = "Expected `string_type` to be one of: {}, got: {{!r}}.".format(_VALID_STRING_TYPES)
_INVALID_ELEMENT_TYPE = "Expected all elements to be of type: %s. Got value: %r of type: %s."
...
class StringArray(Array):
    ...
    _REPR_TEMPLATE = "{self.__class__.__name__}(shape={self.shape}, string_type={self.string_type}, name={self.name})"
    ...
    def validate(self, value):
        ...
        for item in value.flat:
            if not isinstance(item, self.string_type):
                self._fail_validation(_INVALID_ELEMENT_TYPE, self.string_type, item, type(item))
        return value
